...
What we saw in Example 1.1.1 is one of the most widely used _programming patterns_:
...
3. Write a program that shows on the micro:bit 21 numbers, one after the other, starting at zero and increasing in _magnitude_, with the _even_ numbers in the interval [0, 20] are _positive_ and the _odd_ numbers in the same interval are _negative_. _Hint: Consider using a `for` loop with the _less-than-or-equal_ operator `<=` in the condition expression._  
...
3. Write a program like Example 2.3 above, which instead breaks on the value of the outer-loop variable `x`.  
...
Loops and arrays are often used together as the loop variable can be used as the index into the array:
```javascript
// Example 3.1.6
...
The Boolean functions AND, OR, and NOT, fundamental to computing and used at every level, from high-level programming to _logic gates_, can best be expressed with tables, called _truth tables_, as follows:
